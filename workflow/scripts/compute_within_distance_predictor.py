...
def compute_within_distance(line, dist, cell_type, dist_index, annot):
  
  # split line into different fields 
  entry = line.rstrip().split('\t')
  
  # compute within distance to TSS/gene predictor score
  score = int(entry[dist_index]) <= dist * 1000 and entry[9] in annot
  score = str(score * 1)
...
if __name__ == '__main__':
  ...
  parser.add_argument('-a', '--annot', help = 'Bed file containing gene/TSS annotations. In \
    addition to within a given distance a gene needs to be on this list (e.g. expressed genes).')    
  parser.add_argument('-t', '--type', help = 'Should predictor be computed based on distance to \
    TSS or gene?', default = 'tss')
  ...
  # load annotation file and extract all gene names
  annot = pd.read_csv(args.annot, sep = '\t', header = None, usecols = [3])[3].values
  ...
  if args.inputfile:
    if is_gzipped(args.inputfile):
      with gzip.open(args.inputfile, 'rt') as f:
        print_header()
        for line in f:
          annot_entry = compute_within_distance(line, distance, args.cell_type, dist_index, annot)
          if annot_entry[9] == "1":
